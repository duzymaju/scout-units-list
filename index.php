...
try {
    ...
    $loader->set('manager.cache', new CacheManager($loader->getPath('Cache'), $config->getCacheTtl()))
        ...
        ->set('manager.types', new TypesManager($loader->getPath('Types'), get_locale(), $config->getTypeSet()))
    ...
    $configManager = $loader->get('manager.config');
    if ($configManager) {
        $config = $configManager->get();
        $config->setTypeSet(Config::TYPE_SET_DEFAULT);
        $configManager->save($config);
        trigger_error('There was improper type set defined inside configuration. ' .
            'Default type set was restored.', E_USER_NOTICE);
    }
} catch (Exception $e) {
    trigger_error('Something went wrong with Scout Units List plugin: ' . $e->getMessage(), E_USER_WARNING);
}
